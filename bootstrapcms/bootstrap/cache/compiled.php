...
class RouteCompiler implements RouteCompilerInterface
{
    ...
    private static function compilePattern(Route $route, $pattern, $isHost)
    {
        ...
        return array('staticPrefix' => 'text' === $tokens[0][0] ? $tokens[0][1] : '', 'regex' => self::REGEX_DELIMITER . '^' . $regexp . '$' . self::REGEX_DELIMITER . 'sD' . ($isHost ? 'i' : ''), 'tokens' => array_reverse($tokens), 'variables' => $variables);
    }
...
class Carbon extends DateTime
{
    ...
    protected function resolveCarbon($date = null)
    {
        ...
        if (!$date instanceof DateTime && !$date instanceof DateTimeInterface) {
            throw new InvalidArgumentException('Expected null, string, DateTime or DateTimeInterface, ' . (is_object($date) ? get_class($date) : gettype($date)) . ' given');
        }
